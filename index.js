...
class Car {
  ...
  drive(distance) {
    // tank is empty
    if (this.tank === 0) {
      return `cant drive, tank: ${this.tank}`;
    }
  
    const maxDistance = this.tank * this.milesPerGallon;
    const actualDistance = Math.min(maxDistance, distance);
    const gallonsUsed = actualDistance / this.milesPerGallon;
...
class Instructor extends Lambdasian {
  ...
  updateStudentGrade(student) {
    const toAddOrSub = Math.floor(Math.random() * 100)

    if(student.grade === 0) {
      student.grade += toAddOrSub
    }
